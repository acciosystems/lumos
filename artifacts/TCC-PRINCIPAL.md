...
| MVP                                | Produto minimamente viável adotado como recorte deliberado do trabalho. Reúne os fluxos centrais necessários para apresentar a proposta em funcionamento e servir de base à evolução posterior. Conferir no código o que está implementado; a prioridade P0 nos resumos, isoladamente, não comprova conclusão. |
...
Funcionalidades futuras ou fora do MVP não devem aparecer como entregas atuais, mas integram a visão de evolução da Nossa Causa e devem ser discutidas no TCC. Os responsáveis devem alinhar atores, nomes de entidades, requisitos e nomenclatura de telas antes de finalizar diagramas e descrições dependentes.

### Visão de evolução além do MVP

O MVP representa o primeiro recorte funcional da Nossa Causa, não o limite da proposta concebida para a plataforma. O texto acadêmico deverá apresentar as ideias ainda não implementadas como possibilidades de continuidade, sempre separadas das capacidades verificadas no software:

- **Reputação de organizadores e campanhas:** a proposta considera pontuar cancelamentos de última hora, o cumprimento do prazo de prestação de contas e a qualidade da organização da campanha. A concepção inicial deixou em aberto a escolha entre um período fixo e um cálculo relacionado à quantidade de itens doados. A segunda alternativa aplica-se diretamente às campanhas físicas; não foi definido um critério variável equivalente para campanhas virtuais. O MVP analisado adotou sete dias corridos para as duas modalidades. Uma evolução deverá decidir se mantém a regra única ou estabelece critérios por modalidade. A concepção também classifica o descumprimento do prazo como infração dos termos da plataforma, embora o MVP não aplique penalidade automática.
- **Denúncias:** está previsto um canal para reportar campanhas ou organizadores considerados problemáticos.
- **Recompensas pela participação:** a ideia associa um identificador de participação a cada usuário, o que permitiria criar rankings e premiações definidas pelos organizadores.
- **Notificações de campanha:** foram concebidos avisos automáticos sobre as campanhas, inclusive para comunicar alterações. A implementação foi considerada tecnicamente inviável dentro do escopo do MVP, mas o recurso permanece como possibilidade de evolução posterior.
- **Processamento de pagamentos:** uma etapa posterior poderá oferecer integração opcional com serviços como Stripe ou Polar. Essa alternativa complementaria o modelo de transferência direta por PIX ou conta bancária, sem substituir obrigatoriamente a modalidade prevista no MVP.

Essas possibilidades podem ser organizadas, para fins de análise, em três frentes de evolução: confiança e governança, com reputação, denúncias e regras para prestação de contas; participação e engajamento, com recompensas e notificações; e ampliação das formas de contribuição financeira, com a integração opcional de processadores de pagamento. Essa organização é uma leitura analítica do escopo do produto, não uma afirmação de implementação, prazo ou compromisso de entrega.
...
| D-024 | Apresentar no TCC a visão de evolução da Nossa Causa além do MVP, incluindo reputação, denúncias, recompensas, notificações e integração opcional com processadores de pagamento. Esses recursos devem ser discutidos como ideias ainda não implementadas, sem códigos internos de prioridade, datas ou compromissos de entrega. O texto deverá separar os sete dias corridos aplicados às duas modalidades no MVP da política futura: a alternativa baseada na quantidade de itens atende diretamente às campanhas físicas, não possui equivalente definido para as virtuais e deverá ser conciliada com a classificação do atraso como infração dos termos. O MVP não aplica penalidade automática. A situação das notificações foi posteriormente definida por D-029. | Esclarecimento explícito do usuário, escopo registrado em `docs/nossa-causa-resumo-pt.md` e implementação analisada na Fase 3. | Introdução, Fases 3, 4 e 8, considerações finais e integração | Codex / 2026-09-07 |
| D-025 | Apresentar o MVP como uma escolha deliberada de produto minimamente viável, capaz de executar os fluxos centrais da Nossa Causa e estabelecer uma base concreta para evolução. As fronteiras do recorte devem permanecer precisas, mas não podem ser descritas como erro, fracasso, atraso ou simples consequência de falta de tempo. Priorizar a contribuição do que foi implementado antes de explicar funções reservadas a etapas posteriores. | Esclarecimento explícito do usuário. | Todas as partes e integração | Codex / 2026-09-07 |
| D-026 | Incluir na Fase 4 uma única seção de dicionário de dados com uma tabela para cada modelo persistido definido no Prisma, inclusive os modelos técnicos que não aparecem no DER conceitual. As tabelas permanecerão no mesmo tópico, sem capítulos ou subtópicos individuais e sem uma discussão sobre a evolução de suas estruturas. Cada tabela terá as colunas Campo, Descrição, Tipo e Tamanho. Os nomes dos campos serão escritos em maiúsculas e usarão `@` para chave primária, `#` para chave estrangeira e nenhum prefixo para os demais campos. A descrição de todo campo enumerado apresentará seus valores possíveis. Os tipos serão reproduzidos como aparecem no SQL; os tamanhos serão conferidos no schema e nas migrações, e limites não definidos não serão inventados. A exceção posterior referente a `DONATIONS` está registrada em D-029. | Esclarecimentos explícitos do usuário nesta tarefa. | Fase 4 e integração | Codex / 2026-09-08 |
| D-027 | Manter a consolidação e a normalização da lista final de referências como última etapa da revisão do TCC. Durante as revisões intermediárias, qualquer inclusão, remoção ou deslocamento de uso bibliográfico continuará exigindo a atualização imediata do mapa central definido em D-015. | Esclarecimento explícito do usuário. | Todas as partes, referências bibliográficas e integração | Codex / 2026-09-08 |
| D-028 | Não inserir ou renderizar os diagramas nos arquivos Markdown durante a revisão textual. Os arquivos-fonte existentes serão preservados, e a inserção dos diagramas ocorrerá somente no documento final, produzido em outro formato. A ausência das imagens dos diagramas no Markdown não constitui pendência textual. | Esclarecimento explícito do usuário. | Fases 4 e 6; integração e diagramação final | Codex / 2026-09-08 |
| D-029 | Excluir do dicionário textual a tabela `DONATIONS`, sem alterar o código, pois nenhum fluxo operacional da aplicação utiliza essa estrutura no recorte auditado. O dicionário passa a reunir 16 tabelas e 162 campos. Considerar as notificações automáticas tecnicamente inviáveis dentro do escopo do MVP, mas preservá-las como possibilidade de evolução posterior; sua viabilidade não permanece em avaliação para a entrega atual. | Esclarecimentos explícitos do usuário durante o pacote R-02 e conferência do repositório. | Fases 3, 4, 6 e 8; considerações finais; documentos de escopo e integração | Codex / 2026-09-08 |
| D-030 | Normalizar a lista final de referências pela ABNT NBR 6023:2025, preservando o sistema autor-data adotado no texto conforme a ABNT NBR 10520:2023. As referências formarão uma lista única, sem numeração, em ordem alfabética pelo elemento de entrada. Datas ausentes não serão representadas por `s.d.`: deverão ser confirmadas ou registradas por estimativa justificada entre colchetes. | Instrução explícita do usuário e edições vigentes das normas conferidas em 2026-09-08. | Todas as partes, referências bibliográficas e integração | Codex / 2026-09-08 |
...
| Introdução — `introducao`                     | Codex       | Concluído    | `artifacts/planejamento-introducao.md`; `artifacts/texto-introducao.md` | D-007 aplicada: AccioLabs é apresentada como empresa ficcional criada exclusivamente para o TCC. Redação revista em 2026-09-07 conforme D-023, D-024 e D-025, com distinção entre o MVP, sua contribuição funcional e sua visão de evolução; revalidar a coerência com as fases durante a integração final. |
| Fase 1 — `fase-01`                            | Codex      | Concluído      | `artifacts/planejamento-fase-01.md`; `artifacts/texto-fase-01.md`      | Pacote R-01 concluído em 2026-09-08. A fase concentra a justificativa da necessidade, preserva os limites das fontes e identifica Facebook e WhatsApp como parte do problema definido pelo projeto, não como achado empírico.                                                                                     |
| Fase 2 — `fase-02`                            | Codex       | Concluído       | `artifacts/planejamento-fase-02.md`; `artifacts/texto-fase-02.md`    | Pacote R-01 concluído em 2026-09-08. A fase concentra amostras, dados e classificação das evidências, distingue definição documental de inferência e mantém a eficácia da solução fora dos resultados demonstrados.                                                                                  |
| Fase 3 — `fase-03`                            | Codex       | Concluído      | `artifacts/planejamento-fase-03.md`; `artifacts/texto-fase-03.md`    | Planejamento, redação, conferência técnica e revisão concluídos em 2026-09-05. A fase analisa o MVP implementado da Nossa Causa, descreve seus controles pelas funcionalidades P0 e não cita o documento interno que enumera essas prioridades. Em 2026-09-07, D-024 ampliou a continuidade funcional e D-025 consolidou o MVP como recorte deliberado e funcional. A verificação de tipos foi concluída com êxito. |
| Fase 4 — `fase-04`                            | Codex       | Concluído | `artifacts/planejamento-fase-04.md`; `artifacts/texto-fase-04.md`; fontes Mermaid em `artifacts/diagramas/` | D-026 foi concluída em 2026-09-08 e ajustada por D-029. O dicionário reúne 16 tabelas e 162 campos; `DONATIONS` foi excluída do texto por não possuir fluxo operacional no recorte auditado. Chaves, tipos SQL, limites textuais e valores enumerados foram conferidos no schema e nas migrações. A Figura 2 apresenta uma visão lógica da arquitetura; a exportação dos diagramas continua adiada para a etapa condicional de diagramação final. |
| Fase 5 — `fase-05`                            | Codex       | Concluído | `artifacts/planejamento-fase-05.md`; `artifacts/texto-fase-05.md` | Planejamento, redação, conferência e revisão concluídos em 2026-09-06. Conforme D-003, a fase descreve os requisitos do usuário final por capacidades necessárias, sem inventar configurações mínimas nem incluir requisitos de hospedagem. O enquadramento do MVP foi revisto conforme D-025 em 2026-09-07. |
| Fase 6 — `fase-06`                            | Codex       | Concluído | `artifacts/planejamento-fase-06.md`; `artifacts/texto-fase-06.md`; `artifacts/imagens/fase-06/formularios/`; `artifacts/diagramas/fase-06-classes.mmd` | Partes 0 a 6 concluídas e revisadas em 2026-09-07. A fase reúne o padrão do sistema, o inventário de quinze telas, dezenove capturas de formulários, o diagrama de classe e o tópico TSDoc delimitado pela premissa de D-014. A auditoria documental incluiu OWASP, WebAuthn e chamadas TSDoc diferenciadas. D-025 foi aplicada ao enquadramento do MVP. Conforme D-017, ferramenta, comando e saída TSDoc não são apresentados como evidências verificadas; a imagem única do tópico permanece reservada à integração visual. |
| Fase 7 — `fase-07`                            | Codex       | Concluído | `artifacts/planejamento-fase-07.md`; `artifacts/texto-fase-07.md`; `artifacts/manual-do-usuario.md`; `artifacts/auxiliares/fase-07-evidencias.md` | Fase implementada em 2026-09-07. O texto consolida cronograma, implantação teórica, treinamento, segurança e recuperação; o manual é uma entrega real, revisada contra o snapshot, sem alegação de teste prático ponta a ponta. D-025 foi aplicada para apresentar o MVP como base funcional do cenário. Vercel e Neon permanecem targets codificados. Nenhuma implantação, treinamento, backup ou restauração foi declarada como executada. |
| Fase 8 — `fase-08`                            | Codex       | Concluído | `artifacts/planejamento-fase-08.md`; `artifacts/texto-fase-08.md`; `artifacts/auxiliares/fase-08-evidencias.md` | Fase concluída em 2026-09-07. D-021 separa manutenção de software das funções operacionais da Fase 7. O texto distingue recursos verificados, ausências e atividades propostas; a classificação e o processo foram conferidos na ISO/IEC/IEEE 14764:2022 e no SWEBOK Guide V4.0a, registrados no mapa central. D-024 relaciona o processo às ideias posteriores ao MVP, e D-025 apresenta a versão como base funcional para manutenção. Lint e type-check passaram; a ausência de suíte de testes e CI rastreados e o resultado da auditoria de dependências permanecem limites explícitos. |
| Considerações finais — `consideracoes-finais` | Codex       | Concluído | `artifacts/planejamento-consideracoes-finais.md`; `artifacts/texto-consideracoes-finais.md` | Planejamento, redação e revisão concluídos em 2026-09-08. O texto apresenta o MVP como decisão deliberada e base funcional documentada, concentra suas fronteiras na separação de responsabilidades e retoma a continuidade definida em D-024 sem prazo ou compromisso de entrega. O uso indireto das fontes da síntese inicial foi registrado no mapa central; não foram incluídas alegações de impacto ou operação não demonstrados. |
| Referências bibliográficas — `referencias`    | Codex       | Concluído | `artifacts/planejamento-referencias.md`; `artifacts/auxiliares/referencias-utilizadas.md`; `artifacts/referencias.md` | Lista final concluída em 2026-09-08 conforme D-030. As 26 fontes externas foram auditadas nos dois sentidos e organizadas em lista única, sem numeração e em ordem alfabética pela ABNT NBR 6023:2025. As 14 datas provisórias foram resolvidas, os DOI foram normalizados e as chamadas institucionais e letras de 2026 foram sincronizadas conforme a ABNT NBR 10520:2023. A apresentação institucional permanece sujeita ao eventual manual mencionado em P-003. |
...
O dicionário de dados deverá conter, no mesmo tópico e sem subdivisões individuais, uma tabela para cada modelo persistido definido no Prisma que participe dos fluxos do MVP, inclusive os modelos técnicos omitidos do DER conceitual. Conforme D-029, `DONATIONS` não será reproduzida porque nenhum fluxo operacional utiliza essa estrutura no recorte auditado. Não será incluída uma discussão sobre a evolução das estruturas das tabelas. Cada tabela terá as colunas **Campo**, **Descrição**, **Tipo** e **Tamanho**. Na coluna **Campo**, o nome será escrito integralmente em maiúsculas e receberá o prefixo `@` quando for chave primária, `#` quando for chave estrangeira e nenhum prefixo nos demais casos. A descrição dos campos enumerados informará todos os valores possíveis. Os tipos serão reproduzidos conforme o SQL, e os tamanhos corresponderão ao schema e às migrações do banco de dados; quando não houver limite definido, essa ausência será indicada sem adoção de um valor arbitrário.

Além dos diagramas limitados ao MVP, a discussão do sistema proposto deverá explicar como reputação, denúncias, recompensas, notificações e processamento opcional de pagamentos ampliariam a concepção da plataforma. O texto deverá preservar a condição de ideias não implementadas e as delimitações registradas em D-024 e D-029.
...
- Relacionar o processo de evolução do software às ideias posteriores ao MVP registradas em D-024, sem pressupor aprovação, ordem de execução ou prazo.

### Considerações finais

- Retomar o problema, o trabalho desenvolvido, os resultados sustentados e a contribuição do MVP como produto minimamente viável.
- Explicar as fronteiras deliberadas desse recorte e sua função como base para a evolução do produto, sem caracterizá-las como falhas ou consequências de falta de tempo.
- Apresentar as ideias não implementadas como continuidade possível da Nossa Causa, explicando sua relação com confiança, governança, participação, engajamento e formas de contribuição.
- Registrar que reputação, denúncias, recompensas, notificações e processamento opcional de pagamentos permanecem fora da versão analisada.
...
| P-003 | Referências | A instituição possui manual ou modelo próprio para apresentação do TCC que deva ser aplicado junto às normas ABNT? | Codex / 2026-09-08 | Aguardando esclarecimento; não bloqueia o planejamento bibliográfico | Na ausência do manual, a lista será normalizada pelas edições vigentes da ABNT. Fonte, margens e outras escolhas de diagramação permanecerão sujeitas a conferência antes da exportação final. |
