...
- **Notificações de campanha:** foram concebidos avisos automáticos sobre as campanhas, inclusive para comunicar alterações. A viabilidade técnica e a complexidade de implementação ainda precisam ser avaliadas antes da definição do escopo dessa funcionalidade.
...
| Fase 4 — `fase-04`                            | Codex       | Concluído | `artifacts/planejamento-fase-04.md`; `artifacts/texto-fase-04.md`; fontes Mermaid em `artifacts/diagramas/` | D-026 foi concluída em 2026-09-08 com um único tópico de dicionário de dados, composto por 17 tabelas e 173 campos persistidos. Chaves, tipos SQL, limites textuais e valores enumerados foram conferidos no schema e nas migrações. Os diagramas e a discussão de evolução permanecem válidos; a exportação dos diagramas continua adiada para a etapa condicional de diagramação final. |
...
O dicionário de dados deverá conter, no mesmo tópico e sem subdivisões individuais, uma tabela para cada modelo persistido definido no Prisma, inclusive os modelos técnicos omitidos do DER conceitual. Não será incluída uma discussão sobre a evolução das estruturas das tabelas. Cada tabela terá as colunas **Campo**, **Descrição**, **Tipo** e **Tamanho**. Na coluna **Campo**, o nome será escrito integralmente em maiúsculas e receberá o prefixo `@` quando for chave primária, `#` quando for chave estrangeira e nenhum prefixo nos demais casos. A descrição dos campos enumerados informará todos os valores possíveis. Os tipos serão reproduzidos conforme o SQL, e os tamanhos corresponderão ao schema e às migrações do banco de dados; quando não houver limite definido, essa ausência será indicada sem adoção de um valor arbitrário.

Além dos diagramas limitados ao MVP, a discussão do sistema proposto deverá explicar como reputação, denúncias, recompensas, notificações e processamento opcional de pagamentos ampliariam a concepção da plataforma. O texto deverá preservar a condição de ideias não implementadas e as questões ainda abertas registradas em D-024.
