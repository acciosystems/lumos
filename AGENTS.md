...
- Canonical tracker: Linear project `38c62f35-8da5-4606-af07-0554669c2481` (LUMOS), team `Accio Systems Co.` (`ACC`). Treat deleted Linear issues as nonexistent: ignore stale references to them when checking for duplicates, inferring active work, or creating blocking/related issue relationships. When starting an issue, move it to **In Progress**. When committing or drafting a commit message, include Linear's magic word + issue ID (e.g. `Resolves ACC-123`) when an issue exists.
