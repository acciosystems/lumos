...
Today, physical-item donation campaigns rely on informal social media groups, making coordination, tracking, and accountability difficult. The platform centralizes this process, offering engagement statistics, automatic updates on location/time changes, and public participation data. For monetary donations, organizers provide their own payment details (PIX key, bank account number), avoiding payment-processor fees — relying only on their own bank's fees.
...
|                            | Notification system           | Automated campaign alerts — technical feasibility still under evaluation (considered complex to implement) |
...
- **Notifications**: assess implementation complexity before confirming as an MVP feature.
