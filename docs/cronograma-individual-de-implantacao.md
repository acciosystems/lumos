...
| `D-4`          | Geração da versão                 | Instalar dependências com o lockfile do projeto; executar lint, verificação de tipos e build; corrigir falhas impeditivas; identificar a versão candidata à produção.                                                                          |            5 h | Build reproduzível e verificações técnicas concluídas sem erros impeditivos.   | Responsável técnico pelo projeto |
...
2. conexão segura com o PostgreSQL e migrações aplicadas com sucesso;
3. variáveis obrigatórias disponíveis por meio do Doppler;
4. autenticação, recuperação de acesso e configuração inicial funcionando;
5. listagem, filtros e detalhes das campanhas funcionando;
6. criação de campanhas físicas e virtuais validada com perfil organizador elegível;
7. envio de e-mail, armazenamento S3 e logging validados;
8. backup e procedimento de retorno conferidos;
9. inexistência de incidente crítico ou de alta prioridade sem tratamento;
10. treinamento da equipe interna concluído.
